...
**Write the test first when fixing bugs.** Before you fix anything, write a test that reproduces the bug. Run it. Watch it fail. Then fix the bug. Run the test. Watch it pass. This is not optional and not TDD dogma. It's the only way to prove the thing and didn't just make the symptoms go away.
...
How you communicate about the code matters as much as the code itself.
...
## Daniekeys Studios Revamp (active)
Spec lives in docs/revamp/. Read docs/revamp/00-OVERVIEW.md and
...
the authoritative visual reference for layout, spacing, proportions, shapes,
component structure, and responsive behavior.

For every task in this revamp:
1. Before building or editing any section, open and inspect the relevant
   part of /reference/desktop-mockup.jpeg and /reference/mobile-mockup-view.jpg.
2. Build to match them as closely as possible — spacing, alignment, sizing,
   grid structure, card shapes, image proportions, component layout.
3. Do NOT use any color from the reference images. All color must come from
   docs/revamp/01-DESIGN-SYSTEM.md's token set (Black/White/Off White/Light
   Dark/DK_BLUE 1-3). Treat the mockups as if they were grayscale.
4. After building, do a explicit side-by-side cross-check: describe what you
   see in the reference vs. what you built for that section, and call out
